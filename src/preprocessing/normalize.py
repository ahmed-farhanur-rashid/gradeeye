...
def compute_dataset_stats(image_paths: list[str], sample_limit: int | None = None) -> dict:
    """
    Compute per-channel mean/std (in [0, 1] scale, BGR order matching
    cv2 loading convention) over a list of preprocessed image paths.
...
def normalize_image(img: np.ndarray, mean: list[float], std: list[float]) -> np.ndarray:
    """
    Normalize a BGR uint8 image to a float32 tensor-ready array using the
    given per-source mean/std (both in BGR order, [0,1] scale).
    """
