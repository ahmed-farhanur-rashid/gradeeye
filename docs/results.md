...
- **Bootstrap confidence intervals for all reported metrics.** ~~None of the per-fold QWK, accuracy, F1, AUC, or per-threshold ECE numbers yet carry a paired-bootstrap 95% confidence interval. The procedure is defined in the methodology §Statistical Significance. The eval runner does not currently persist per-sample logits, so the implementation requires extending the JSONL output schema (or re-running the eval pipeline with logits dumped) before the bootstrap can be applied. Estimated compute: ~60–90 minutes on the four ConvNeXt-Tiny checkpoints plus the ablation matrix, after the eval-pipeline change.~~ **Done 2026-08-12.** Implemented 1000-resample paired bootstrap on per-sample predictions dumped to `saved/logs/per_sample_predictions/` (5 variants × 4 folds = 20 inference runs, no retraining; pre-sample logits recovered from per-threshold-probs via inverse-sigmoid for downstream calibration). Bootstrap-CI results in `saved/logs/bootstrap_cis.json`. Concrete finding: on APTOS the 3ch-balanced baseline QWK 0.832 [0.822, 0.842], 4ch soft 0.862 [0.851, 0.872]; the 3–4 percentage-point APTOS gain is statistically meaningful at α=0.05 on every variant. On EyePACS every variant is CI-overlapped (0.42–0.46), the segmentation gain is not statistically significant.
