...
The decision-boundary tuning analysis below sweeps an additive bias $b \in [-0.40, +0.40]$ (step 0.01) applied uniformly to all four CORN sigmoid threshold probabilities, then maps to the categorical class via the CORN joint probability and computes QWK. Bias $b = 0$ recovers the default argmax rule; $b > 0$ softens every threshold (predicts higher grade), $b < 0$ sharpens every threshold (predicts lower grade). The "tuned QWK" reported is the mean of 5 honest 50/50 splits stratified by class — the bias is fit on one half of the held-out test set and evaluated on the other half, removing the bias-selection train/test leak that picking $b$ on the same eval set would introduce. No logits are modified, no model weights are changed.

Per-variant per-fold results (full per-($b$) sweep curves in `docs/analysis/figures/decision_boundary/`):

| Variant | Held-out source | Baseline QWK | Tuned QWK (OOB-CV) | ΔQWK | Best bias $b$ |
|:--------|:----------------|:------------:|:------------------:|:----:|:-------------:|
| 3ch balanced | EyePACS | 0.4399 | 0.4426 | +0.0026 | -0.02 |
| 3ch balanced | APTOS | 0.8323 | 0.8765 | **+0.0442** | -0.19 |
| 3ch balanced | Messidor-2 | 0.7261 | 0.7675 | +0.0413 | +0.23 |
| 3ch balanced | DDR | 0.7466 | 0.7542 | +0.0076 | +0.09 |
| 3ch unbalanced | EyePACS | 0.4492 | 0.5389 | **+0.0897** | -0.23 |
| 3ch unbalanced | APTOS | 0.8607 | 0.8704 | +0.0097 | -0.17 |
| 3ch unbalanced | Messidor-2 | 0.7661 | 0.7675 | +0.0014 | -0.01 |
| 3ch unbalanced | DDR | 0.7759 | 0.7805 | +0.0046 | +0.12 |
| 4ch soft | EyePACS | 0.4601 | 0.4620 | +0.0020 | -0.02 |
| 4ch soft | APTOS | 0.8615 | 0.8890 | **+0.0274** | -0.16 |
| 4ch soft | Messidor-2 | 0.6664 | 0.7121 | +0.0456 | +0.22 |
| 4ch soft | DDR | 0.7600 | 0.7578 | −0.0021 | +0.01 |
| 4ch Tversky | EyePACS | 0.4256 | 0.4529 | +0.0272 | -0.10 |
| 4ch Tversky | APTOS | 0.8082 | 0.8556 | **+0.0473** | -0.19 |
| 4ch Tversky | Messidor-2 | 0.7021 | 0.7677 | +0.0656 | +0.26 |
| 4ch Tversky | DDR | 0.7347 | 0.7567 | +0.0220 | +0.18 |
| 4ch morph | EyePACS | 0.3668 | 0.3824 | +0.0156 | -0.14 |
| 4ch morph | APTOS | 0.8598 | 0.8685 | +0.0087 | -0.17 |
| 4ch morph | Messidor-2 | 0.6742 | 0.7164 | +0.0422 | +0.12 |
| 4ch morph | DDR | 0.7590 | 0.7583 | −0.0007 | +0.08 |

**Cross-variant mean ΔQWK (4-fold mean):**

| Variant | Mean baseline QWK | Mean tuned QWK | Mean ΔQWK |
|:--------|:-----------------:|:--------------:|:---------:|
| 3ch balanced   | 0.6862 | 0.7102 | +0.0239 |
| 3ch unbalanced | 0.7130 | 0.7393 | +0.0264 |
| 4ch soft       | 0.6870 | 0.7052 | +0.0182 |
| 4ch Tversky    | 0.6676 | 0.7082 | **+0.0405** |
| 4ch morph      | 0.6650 | 0.6814 | +0.0164 |

**Interpretation:** Every (variant, fold) cell sees a non-negative ΔQWK except two (4ch soft × DDR and 4ch morph × DDR, both essentially flat). The largest single gain is 3ch unbalanced × EyePACS (+0.0897, bias $b = -0.23$); this is consistent with the unbalanced baseline's tendency to predict higher grades on the EyePACS-prior-heavy fold — sharpening the thresholds (negative bias) recoups those over-predictions. The decision-boundary procedure is a strictly post-hoc decision-rule change — no model retraining, no logit modification, no calibration. The cross-variant gain is largest on 4ch Tversky (+0.0405), suggesting that variant's biases are most amenable to post-hoc correction. The honest 50/50 OOB-CV reported ΔQWK is a conservative lower bound on the in-sample optima; the table in §4 of the analysis shows the in-sample best, which is materially higher (e.g. +0.18 in-sample for 3ch unbalanced × EyePACS). The full per-($b$) sweep curves are in `docs/analysis/figures/decision_boundary/`.
...
- **Decision-boundary threshold tuning (§7 of this document).** ~~The validation-tuned ordinal thresholds place decision boundaries on the expected-grade score to maximize QWK. The §7 table is not yet populated.~~ **Done 2026-08-12.** Swept additive bias $b \in [-0.40, +0.40]$ on all four CORN sigmoid thresholds with honest 50/50 OOB-CV (5 reps, stratified by class) for all five ConvNeXt-Tiny variants × four folds. §7 is populated with per-(variant, fold) baseline vs tuned QWK, mean ΔQWK per variant, and a finding that **every cell sees non-negative ΔQWK except two** (mean cross-variant ΔQWK +0.025). No retraining, no logit modification — pure decision-rule post-processing. Full per-($b$) sweep curves in `docs/analysis/figures/decision_boundary/`.
...
- **Bootstrap confidence intervals for all reported metrics.** ~~None of the per-fold QWK, accuracy, F1, AUC, or per-threshold ECE numbers yet carry a paired-bootstrap 95% confidence interval. The procedure is defined in the methodology §Statistical Significance. The eval runner does not currently persist per-sample logits, so the implementation requires extending the JSONL output schema (or re-running the eval pipeline with logits dumped) before the bootstrap can be applied. Estimated compute: ~60–90 minutes on the four ConvNeXt-Tiny checkpoints plus the ablation matrix, after the eval-pipeline change.~~ **Done 2026-08-12.** Implemented 1000-resample paired bootstrap on per-sample predictions dumped to `saved/logs/per_sample_predictions/` (5 variants × 4 folds = 20 inference runs, no retraining; per-sample logits recovered from per-threshold probabilities via inverse-sigmoid for downstream calibration). Bootstrap-CI results in `saved/logs/bootstrap_cis.json`. Concrete finding: on APTOS the 3ch-balanced baseline QWK 0.832 [0.822, 0.842], 4ch soft 0.862 [0.851, 0.872]; the gain is statistically meaningful at α=0.05. EyePACS is also significant for 4ch soft versus 3ch balanced: 0.460 [0.453, 0.467] versus 0.440 [0.433, 0.448], with non-overlapping intervals. The DDR gain is not significant; the Messidor-2 regression is significant and in the wrong direction.
- **Per-threshold calibration of the auxiliary-channel variants.** ~~The per-threshold calibration analysis is run on the 3-channel baseline only in this study. Re-running on the 4-channel soft, Tversky, and morphological variants would test whether the auxiliary-channel question and the per-threshold calibration question interact.~~ **Done 2026-08-12.** Extended the per-threshold and global temperature scaling diagnostic to the 3-ch unbalanced, 4-ch soft, 4-ch Tversky, and 4-ch morph variants. Honest 50/50 5-rep CV (temperature fitted on train half of held-out test, ECE evaluated on test half). Full results in `saved/logs/calibration_all_variants.json` and `saved/logs/calibration_summary.md`. Key finding: **the global-temperature-scaling pathology is identical across all five variants** — every fit saturates at $T = 5.0$ (the optimizer's upper bound) on the unbalanced baseline and the morph variant on every fold, and the global-T ECE is consistently *worse* than raw ECE on every (variant, fold) pair except one. Per-threshold scaling continues to give small gains (mean ΔECE −0.015 across variants) but no longer saves the global-T pathology.
