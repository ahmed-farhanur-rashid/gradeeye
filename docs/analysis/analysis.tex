...
\begin{itemize}[leftmargin=*, topsep=0pt]
  \item \textbf{Bootstrap confidence intervals for all reported metrics.} \emph{Completed 2026-08-12.} Per-sample predictions dumped to \texttt{saved/logs/per\_sample\_predictions/} for all five ConvNeXt-Tiny variants $\times$ four folds (20 inference runs, no retraining). 1000-resample paired bootstrap on the per-sample arrays yields the 95\% confidence intervals for QWK, accuracy, and macro-F1 reported alongside the per-fold point estimates in §2--§4 and §6 of the results summary. JSON artifact: \texttt{saved/logs/bootstrap\_cis.json}. Concrete observation: on APTOS the segmentation gain is statistically meaningful at $\alpha = 0.05$ (4-ch soft 0.862 [0.851, 0.872] vs 3-ch balanced 0.832 [0.822, 0.842], non-overlapping CIs); on EyePACS it is not (3-ch baseline 0.440 [0.432, 0.448] vs 4-ch soft 0.460 [0.453, 0.467], overlapping CIs).
  \item \textbf{Decision-boundary threshold tuning (§7 of the results summary).} \emph{Completed 2026-08-12.} Post-hoc additive bias $b \in [-0.40, +0.40]$ on all four CORN sigmoid thresholds with honest 50/50 out-of-bias-sample CV (5 reps, stratified by class). Mean cross-variant $\Delta$QWK is $+0.025$; every (variant, fold) cell except two sees non-negative $\Delta$QWK. Largest single gain is 3-ch unbalanced $\times$ EyePACS ($+0.0897$, bias $b = -0.23$). No logits or weights are modified. Full per-($b$) sweep curves in \texttt{docs/analysis/figures/decision\_boundary/}.
