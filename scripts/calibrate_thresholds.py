"""
Calibrate per-class thresholds on a held-out calibration set and evaluate
the gain on test sets.

This is post-training, no-fine-tune threshold calibration:
1. Run trained model on calibration manifest (default: EyePACS val) →
   collect (y_proba, y_true) pairs.
2. Optimize 4 thresholds (between 5 classes) to maximize QWK on those.
3. Save calibrated thresholds to JSON.
4. Report QWK with argmax vs. QWK with calibrated thresholds on:
   - the calibration set (sanity check, can over-fit)
   - APTOS test (in-domain evaluation)
   - Messidor2 (out-of-domain, this is where calibration shines)

Usage:
    python scripts/calibrate_thresholds.py \
        --checkpoint saved/checkpoints/full_method_convnext_best.pt \
        --calib-manifest data/splits/eyepacs_val.csv \
        --thresholds-out saved/checkpoints/calibrated_thresholds.json

Default flags include the two test manifests + EMA + no TTA. Pass --tta
to also run the calibrated version with TTA.

Why this is safe:
- It does NOT modify the trained model or its weights.
- Calibration on val, evaluated on test — no leak.
- The EyePACS val used for calibration is the distribution the model was
  trained to maximize likelihood on, so it's the right set for fitting the
  decision rule; APTOS/Messidor2 test sets are used only for measuring
  transfer, never for tuning.
"""
import argparse
import datetime
import json
...
# Reuse the ensemble's loader (handles CORN/CE + channels_last + in_chans etc.)
from scripts.ensemble_evaluate import load_model_from_checkpoint, _strip_compile_prefix


def collect_probas(model, manifest_csv: str, batch_size: int, device: str,
                    use_tta: bool = False, img_size: int | None = None,
                    seg_dir: str | None = None) -> tuple[np.ndarray, np.ndarray]:
    """Run model on manifest, return (y_proba [N,5], y_true [N])."""
    ds = DRDataset(manifest_csv, transform=build_eval_transforms(img_size=img_size),
                    seg_dir=seg_dir)
    ...
    with torch.no_grad():
        for images, labels in dl:
            ...
            if use_tta:
                ...
                # tta_forward returns averaged CLASS probabilities
...
def evaluate_with_thresholds(y_proba: np.ndarray, y_true: np.ndarray,
...
def main():
    parser = argparse.ArgumentParser(description="Calibrate decision thresholds on a held-out set.")
    parser.add_argument("--checkpoint", required=True)
    parser.add_argument("--calib-manifest", default="data/splits/eyepacs_val.csv",
                        help="Held-out set used to fit thresholds (default: EyePACS val).")
    parser.add_argument("--thresholds-out",
                        default="saved/checkpoints/calibrated_thresholds.json")
    parser.add_argument("--test-manifests", nargs="+",
                        default=["data/splits/aptos_test.csv",
                                  "data/splits/messidor2_test.csv"],
                        help="Manifests to evaluate gain on (default: APTOS + Messidor2).")
    parser.add_argument("--batch-size", type=int, default=64)
    parser.add_argument("--no-ema", action="store_true")
    parser.add_argument("--tta", action="store_true")
    parser.add_argument("--img-size", type=int, default=None,
                        help="Override input size (uses checkpoint's if not given)")
    ...
    # ── Step 1: collect probabilities on calibration set ──
    ...
    # ── Step 3: report in-domain (calibration set) ──
    print(fmt_section("Step 3a: in-domain evaluation (calibration set — sanity check, may over-fit)"))
    ...
    print(f"  Argmax  QWK: {res_argmax['metrics']['qwk']:.4f}  |  Macro F1: {res_argmax['metrics']['macro_f1']:.4f}")
    print(f"  Thresh  QWK: {res_calib['metrics']['qwk']:.4f}  |  Macro F1: {res_calib['metrics']['macro_f1']:.4f}")
    print(f"  In-domain gain (calibration set): {res_calib['metrics']['qwk'] - res_argmax['metrics']['qwk']:+.4f}")
    print("\n  Argmax confusion matrix:")
    print(format_confusion_matrix_str(res_argmax["confusion_matrix"]))

    # ── Step 4: report on test manifests ──
    ...
    for manifest in args.test_manifests:
        ...
        # Show per-class recall shift for the most common minority (Mild = 1)
        ...
        if not np.isnan(per_class_recall_argmax):
            print(f"    Mild recall: argmax={per_class_recall_argmax:.3f} → calibrated={per_class_recall_thresh:.3f}")
    ...
    print("    evaluate.py (single):  will need a small wrapper — see scripts/evaluate_calibrated.py")
    print("    ensemble_evaluate.py:  ditto")
