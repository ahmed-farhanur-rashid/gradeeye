#!/bin/bash
# Step E: convnext_tiny on unbalanced LODO splits (4 folds).
# This produces the unbalanced baseline to compare against Step B's balanced baseline.
# Uses configs/lodo_unbalanced/<holdout>.yaml instead of configs/lodo_balanced/.
...
ARCHS=(convnext_tiny)
EXPERIMENT_NAME="baseline_3ch_unbalanced"
...
run_one() {
  ...
  local lodo_config="configs/lodo_unbalanced/${holdout}.yaml"
  ...
  echo "================================================================"
  echo "Step E run: arch=${arch}  holdout=${holdout}  retry=${retry}"
  echo "  start: $(date '+%F %T')"
  echo "================================================================"
...
for arch in "${ARCHS[@]}"; do
  for holdout in "${HOLDOUTS[@]}"; do
    total=$((total + 1))
    echo ""
    echo ">>> [Step E ${total}/4] starting ${arch} × ${holdout} <<<"
    run_one "${arch}" "${holdout}"
  done
done
elapsed=$(( $(date +%s) - start_time ))
echo "================================================================"
echo "Step E complete: ${total} runs in $((elapsed / 3600))h $((elapsed % 3600 / 60))m"
echo "================================================================"
